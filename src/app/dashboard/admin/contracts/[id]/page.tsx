...
export default async function AdminContractDetailPage({
  ...
  const paymentProgress = (totalPaid / contract.totalAmount) * 100;
          ...
          <CardContent className="space-y-3">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex items-center gap-2">
                ...
                {contract.provider.user.email}
              </div>
              {contract.provider.user.phone && (
                <div className="flex items-center gap-2">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div>
              ...
              <p className="text-2xl font-bold">{formatPrice(contract.totalAmount)}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">ค่าเช่ารายเดือน</p>
              <p className="text-2xl font-bold">{formatPrice(contract.monthlyAmount)}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">เงินมัดจำ</p>
              <p className="text-2xl font-bold">{formatPrice(contract.depositAmount)}</p>
            </div>
          </div>
            ...
            <TableBody>
              {contract.payments.map((payment, index) => (
                <TableRow key={payment.id}>
                  ...
                  <TableCell>{formatThaiDate(payment.dueDate, "d MMM yyyy")}</TableCell>
